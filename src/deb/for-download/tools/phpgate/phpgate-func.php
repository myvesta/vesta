...
function phpgate_update_timings($arr) {
    ...
    foreach ($arr as $time => $counter) {
        if ($time<$unixtime-$phpgate_ban_time) {unset($arr[$time]); $phpgate_current_ip_counter-=$counter; continue;} // remove expired timings
...
if ($phpgate_the_same_ip==false) {
    ...
    if (isset($_GET['remove_me_from_blacklist'])==false) {
        ...
        if (isset($phpgate_global_banned[$phpgate_ip])) {
            ...
            if (isset($phpgate_global_banned[$phpgate_ip]) && $phpgate_global_banned[$phpgate_ip] < time()-$phpgate_ban_time) {
                // last hit was before 24 hours, unban IP
                phpgate_memcached_safe_update($phpgate_banned_key, 'phpgate_remove_expired_banned', $phpgate_ban_time); // time to unban IP
            } else {
                // last his was in last 24 hours, so add 24 hours to time_counter and keep it blocked
                if ($phpgate_extend_ban_time==true) phpgate_memcached_safe_update($phpgate_banned_key, 'phpgate_add_banned', $phpgate_ban_time);
